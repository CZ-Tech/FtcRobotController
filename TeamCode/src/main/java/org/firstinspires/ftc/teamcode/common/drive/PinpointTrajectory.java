...
public class PinpointTrajectory {
    ...
    public static double P_DRIVE_TURN_GAIN = 0.07;
    ...
    private Mode pathMode = Mode.TIMELINE;
    //endregion
...
//        t += delayTime;
        ...
        if (this.pathMode == Mode.TIMELINE){
            this.t = t;
...
//            robot.telemetry.addData("Odo", robot.odo.getPosition().toString());
        ...
        //run function
        if ((!Globals.DEBUG || DEBUG_RUN_FUNC) && !DO_NOT_RUN_FUNC) TaskLoopFrame.runOnce(fn);
    ...
    public PinpointTrajectory addFunc(Runnable fn){
        if (pathMode == Mode.SEPARATED) return this.addPoint(0, this.x, this.y, this.dx, this.dy, fn);
